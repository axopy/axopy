...
AxoPy's ``pipeline`` subpackage is a small infrastructure for processing data
...
In ``pipeline``, data processing is implemented through a :class:`~.Pipeline`.
...
actual processing involved in each can vary. Some common cases are implemented,
but creating custom blocks and connecting them together in a pipeline structure
is simple. Also, the picture above shows a simple series structure, where each
block takes input only from the block before it. More complex structures are
sometimes convenient or necessary, and some complexity is supported.
...
Features in ``pipeline`` are classes that take all of their parameters in
